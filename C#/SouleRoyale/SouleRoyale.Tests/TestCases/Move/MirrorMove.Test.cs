...
[FeatureFile(@"./Features/Move/MirrorMove.feature")]
public sealed class MirrorMoveTest : Feature
{
#pragma warning disable CS8618 // Un champ non-nullable doit contenir une valeur non-null lors de la fermeture du constructeur. Envisagez de déclarer le champ comme nullable.
    private Player _player;
    private readonly MirrorMovePlayer _mirrorMovePlayer = new();
...
#pragma warning restore CS8618 // Un champ non-nullable doit contenir une valeur non-null lors de la fermeture du constructeur. Envisagez de déclarer le champ comme nullable.
    ...
    {
        _player = new Player(_mirrorMovePlayer);
    }
    ...
    public void I_move_the_player_his_position_change(DataTable dataTable)
    {
        foreach (var row in dataTable.Rows.Skip(1))
        {
            // Given
            _player.Position = 0;
            var moveTypeName = row.Cells.ElementAt(0).Value;
            var expectedPosition = Convert.ToInt32(row.Cells.ElementAt(1).Value);
    ...
    {
        _positionSaved = 3;
        _player = new Player(_mirrorMovePlayer) { Position = 3 };
    }
